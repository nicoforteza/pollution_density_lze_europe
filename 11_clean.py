...
def read_and_clean_within_lze(year):
    ...
    aux = pd.merge(cities.drop('geometry', axis=1),
                   ...
                   on='id', how='left').drop('year', axis=1)
    aux['year'] = year
    aux['timetotreat'] = aux['year'] - aux['yeartreat']
    df['id'] = df['city'].astype(int)
    df = df.drop(['city'], axis=1)
    cols = ['lze', 'lze_1km', 'lze_3km', 'lze_5km', 'lze_10km']
    df[cols] = (df[cols] > 0).astype(int).values
    df['id'] = df['id'].astype(int)
    aux = aux.drop('year', axis=1)
    lzes.drop(['geometry', 'id_uar'], axis=1).drop_duplicates(subset='id').to_excel(r"data/uar/lez_revise.xlsx")
