...
for year in years:
    ...
    # population
    cpop = pop.rio.clip(eur.geometry)
    # pollution
    cpol = pol.rio.clip(eur.geometry)
    # cities id
    cc = make_geocube(
        vector_data=cities,
        measurements=["id"],
        ...
        fill=0
    ).to_array().sel(variable='id').rio.reproject_match(cpol)
    ...
    df.columns = ['pop', 'pol', 'city', 'lat', 'long']
    ...
    df.to_parquet(r"data/within/{}/step1.pqt".format(year))
    ...
    coast_grid = make_geocube(
        ...
        fill=0
    ).to_array().sel(variable='coast').rio.reproject_match(pol)
    ...
    water_grid = make_geocube(
        ...
        resolution=pol.rio.resolution(),
        fill=0
    ).to_array().sel(variable='water').rio.reproject_match(pol)
    ...
    pwgrid = make_geocube(
        ...
        resolution=pol.rio.resolution(),
        fill=0
    ).to_array().sel(variable='power_plant').rio.reproject_match(pol)
    ...
    aux.columns = ['mean_temp', 'mean_precip', 'mean_wind', 'mean_ruggedness',
                   'mean_elevation', 'coast_city', 'water_dist', 'pw_dist']
    df = pd.read_parquet(r"data/within/{}/step1.pqt".format(year))
    df = pd.concat([df, aux], axis=1)
    df.to_parquet(r"data/within/{}/step2.pqt".format(year))
    ...
    aquif = make_geocube(
        ...
        resolution=pol.rio.resolution(),
        fill=0
    ).to_array().sel(variable='AQUIF_CODE').rio.reproject_match(pol)
    ...
    ).sel(band=1).rio.set_crs(4326).rio.clip(eurproj.geometry).rio.reproject_match(pop).rio.clip(eur.geometry)
    ...
    df = pd.read_parquet(r"data/within/{}/step2.pqt".format(year))
    df = pd.concat([df, x], axis=1)
    df.to_parquet(r"data/within/{}/within.pqt".format(year))
